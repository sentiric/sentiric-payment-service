...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-payment-service ./cmd/payment-service
...
COPY --from=builder /app/bin/sentiric-payment-service .
RUN chown appuser:appgroup ./sentiric-payment-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-payment-service"]